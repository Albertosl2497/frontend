...
const [confirmationSentEmails, setConfirmationSentEmails] = useState([]);
 ...
 const phoneNumber = userData.user.phoneNumber.replace(/\s/g, "");
  ...
  const message = `HOLA BUENOS DIAS SOLO PARA RECORDAR QUE EL DIA DE HOY SE LLEVARA ACABO LA RIFA DE LOS $20,000 PESOS💸
  𝗘𝗦𝗧𝗔𝗥𝗘𝗠𝗢𝗦 𝗥𝗘𝗖𝗜𝗕𝗜𝗘𝗡𝗗𝗢 𝗟𝗢𝗦 𝗣𝗔𝗚𝗢𝗦 𝗛𝗔𝗦𝗧𝗔 𝗟𝗔𝗦 4PM 𝗗𝗘𝗟 𝗗𝗜𝗔 𝗗𝗘 𝗛𝗢𝗬.☺️🌸
 ...
 const phoneNumber = userData.user.phoneNumber.replace(/\s/g, "");
  ...
  const message = `HOLA BUENAS TARDES SOLO PARA INFORMAR QUE LA HORA LIMITE DE PAGO SERA HOY A LAS 6:20PM. Si gusta que esperemos un poco mas nos avisa porfa. GRACIAS😊🌼`
 const whatsappUrl = `https://api.whatsapp.com/send?phone=${phoneNumber}&text=${encodeURIComponent(message)}`;
  window.open(whatsappUrl, "_blank");
